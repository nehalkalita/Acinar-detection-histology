...
color_value = 0
color_value2 = -1
output_type = input('Enter your expected type: ')
if output_type == '0':
    output_type_name = 'L1-2_4'
    color_value2 = 85
elif output_type == '1':
    output_type_name = 'L1_4'
elif output_type == '2':
    output_type_name = 'L2_4'
    color_value = 85
elif output_type == '3':
    output_type_name = 'L2_8'
    color_value = 48
elif output_type == '4':
    output_type_name = 'L8_8'
    color_value = 240
else:
    sys.exit()
...
def white_segments(y_axis, x_axis, s_size):
        ...
        if math.floor(section_n / x_total) < (y_total - 1):
            ...
            while (i1 < s_size):
                if (section_n % x_total) < (x_total - 1):    
                    i2 = 0
                    while (i2 < s_size):
                        if (section_items[section_n][i1][i2][0] == color_value) or (section_items[section_n][i1][i2][0] == color_value2):
                            if len(temp_areas) == 0:
    ...
    output_file_open = open(output_type_name + '_combined.csv', 'w', newline='')
    csv_write1 = csv.writer(output_file_open, delimiter = ',')
    output_file_open = open(output_type_name + '_segmented.csv', 'w', newline='')
    ...
    for i in segmented_areas:
        ...
        csv_write2.writerow('')
    return image1, output_type_name

white_s_rtn = white_segments(y_axis, x_axis, s_size)
print('return values:', white_s_rtn)
